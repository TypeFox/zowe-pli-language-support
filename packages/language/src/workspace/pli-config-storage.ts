...
export class PliConfigStorage {
    ...
    constructor(services: LangiumSharedServices) {
        ...
        services.lsp.Connection?.onNotification('pli/pgmConfChanged', async (params) => {
    ...
    async updateConfig(text: string, update = true): Promise<void> {
        try {
            ...
            if (update) {
                ...
                const documents = this.documents.all.filter(doc => doc.uri.scheme !== 'pli-builtin').toArray();
                ...
                await this.documentBuilder.update(documents.map(doc => doc.uri), []);
            }
        } catch (error) {
