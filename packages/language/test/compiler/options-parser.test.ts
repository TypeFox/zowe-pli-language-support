...
import { AbstractCompilerOption, AbstractCompilerOptionString, AbstractCompilerOptionText, isAbstractCompilerOption, isAbstractCompilerOptionString, isAbstractCompilerOptionText, parseAbstractCompilerOptions } from "../../src/compiler/parser";
...
describe('CompilerOptions parser', () => {
    ...
    test('compiler option with nested parameter', () => {
        const options = parseAbstractCompilerOptions("DISPLAY(WTO(ROUTCDE(2)))").options;
        expect(options).toHaveLength(1);
